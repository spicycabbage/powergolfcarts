              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={async()=>{ try { setSaving(true); const res = await fetch(`/api/admin/orders/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:'cancelled'})}); const j=await res.json(); if(!j.success) throw new Error(j.error||'Failed to cancel order'); setOrder((p:any)=>({...p,status:'cancelled'})); toast.success('Inventory returned back to system') } catch(e:any){ toast.error(e?.message||'Failed to cancel order') } finally { setSaving(false) } }} className="px-3 py-2 rounded bg-red-600 text-white hover:bg-red-700 w-full disabled:opacity-50" disabled={saving}>Cancel Order</button>
