...
// List of new 404 URLs to clean up
const urlsToRemove = [
  'https://www.godbud.cc/canadakushies@gmail.com',
  'https://www.godbud.cc/products/pink-kush-cake-shatter',
  'https://www.godbud.cc/products/sour-diesel-rosin',
  'https://www.godbud.cc/products/twisted-zzz-80mg',
...
async function cleanupNew404URLs() {
  try {
    await connectToDatabase();
    console.log('🗑️ Starting cleanup of new 404 URLs...');
    
    // Remove products with these slugs from database
    if (productSlugs.length > 0) {
      console.log(`\n🔍 Checking for products with slugs: ${productSlugs.join(', ')}`);
      
      // First, let's see what products exist with these slugs
      const existingProducts = await Product.find({ slug: { $in: productSlugs } }).lean();
      
      if (existingProducts.length > 0) {
        console.log(`\n📦 Found ${existingProducts.length} products to remove:`);
        existingProducts.forEach(product => {
          console.log(`   - ${product.name} (slug: ${product.slug})`);
        });
        
        const result = await Product.deleteMany({ slug: { $in: productSlugs } });
        console.log(`\n✅ Successfully removed ${result.deletedCount} products from database`);
      } else {
        console.log(`\n✅ No products found with these slugs - they may already be cleaned up`);
      }
    }
